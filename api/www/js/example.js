...
function test() {
    const percentColors = [
        ...
        { percentage: 0.1, color: { r: 0xff, g: 0xff, b: 0x00 } },
    ...
    $.post('/api/v1/parcels?average_sale_price=1', {}, function(data) {
        ...
        for (let zipCode in averageSalePrices) {
            const averageSalePrice = averageSalePrices[zipCode];
            if (averageSalePrice < min) {
                min = averageSalePrice;
            }
            else if (averageSalePrice > max) {
                max = averageSalePrice;
            }
        }
        ...
        searchMap.data.setStyle(function(feature) {
            ...
            if (averageSalePrice) {
                opacity = 0.25;
                strokeWeight = 1;
                const percentage = (averageSalePrice - min) / (max - min);
                console.log(averageSalePrice + " - " + percentage);

                color = getColorForPercentage(percentage, percentColors);
            }
        ...
        searchMap.data.loadGeoJson("/example/data/franklinZipRegions.json");
    });
